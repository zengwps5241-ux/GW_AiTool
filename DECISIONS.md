...
## 决策 #55：M5.5.6 对话「标记为有价值」→ 个人空间知识片段（§2.6 line157 / Phase 4 line1334，后端先行）

- **背景**：规格要求 Chat Mode 自由对话提供「标记为有价值」按钮 → 内容自动存为个人空间对应项目目录下的 Markdown 知识片段（line157 / 资产矩阵 line1126「个人空间/项目名/知识片段/」）。**审计结论**：该项为「规格提及但非 dev-plan 任务」的真缺口——开发计划拆解V2.0.md 对「方法论库/知识片段/标记为有价值/自动归档」5 词零命中，从未拆解为 M-任务；既有 KnowledgeBase（M2.2.4/M2.2.8）是项目级结构化知识库（role_recognition/behavior_quick_ref/onboarding_guide），与对话标记→Markdown 落盘是两回事；代码零实现。抉择：① 标记内容来源（前端持正文 vs 后端回查消息）；② 挂哪个模块；③ 文件名/目录/安全策略；④ 落点归属。
- **选择 A — 内容由前端发送（assistant 正文 copyText），后端不回查消息存储**：消息历史经 `load_history` 依赖真实 claude 会话数据（交接基线标注的 flaky 项），后端按 message_id 回查脆弱且耦合 SDK；前端已持有渲染正文（TurnView.copyText 拼接 text parts），直接 POST `content` 最简且解耦。后端只负责：会话鉴权 + 解析 project_name + 安全文件名 + 落盘。
- **选择 B — 挂 sessions 模块（schema+service+route 全在 reload 舞蹈内），零新 reload 行**：KnowledgeFragmentIn/Out 挂 schemas/sessions.py（Layer2 line72 已 reload），save_knowledge_fragment 挂 modules/sessions/service.py（Layer4 line130 已 reload），POST 端点挂 api/routes/sessions.py（Layer5 line178 已 reload）——**零新 reload 行**，M5.5.1/#52 + M5.5.3/#54 同款手法。sessions 语义合理：标记动作源于会话、需会话上下文解析项目。
- **选择 C — 复用既有写盘工具 + 自定唯一化，不自造路径逻辑**：复用 `user_workspace(username)` 取个人空间根、`resolve_inside_workspace` 做越权校验、`safe_filename` 净化文件名/项目名（防路径穿越 + Windows 非法符，text_ops 同款）。文件名 `{时间戳YYYYMMDD_HHMMSS}_{净化标题}.md`，同秒冲突 while 循环追加 `_N`；目录 `{项目名净化}/知识片段/`，会话未绑项目落根 `知识片段/`；正文前置 HTML 注释记来源会话/项目/时间（复盘用，MarkdownView 渲染不可见）。.md 后缀走 ensure_editable_text_path 既校验。
- **选择 D — 落点恒为标记者（current_user）个人空间，非会话归属**：规格明示「个人空间」，无论会话 personal/team，知识片段总存标记者本人个人空间；会话仅用于鉴权访问（get_accessible_session_svc）+ 解析 project_id→project_name。语义正确：标记是用户个人知识沉淀行为。
- **理由**：审计确认净新范围→前端持正文避开脆弱消息回查；复用 sessions 三层零 reload 风险；既有路径工具 + HTML 注释元数据最小自洽。可校验性：新增 7 测试全过（无项目落根 `知识片段/` + 项目落 `{项目名}/知识片段/` + 纯空白 400 + 未知会话 404 + 未登录 401/403 + 标题 `../../etc/passwd:bad*name?` 净化不逃出工作区 + 同秒重复标记唯一化两文件并存）；test_sessions_project_binding 全过确认 sessions 模块无回归（test_sessions_crud 2 失败为既有 Windows 路径分隔符基线 `user_workspaces\alice`≠`/alice`，与本次无关）。前端 TurnView 传 sessionId + 标记按钮（Book 图标 + 状态机 idle/saving/saved/error）作为跟进子任务。

## 决策 #56：M5.5.7 团队空间「方法论库」— 全局 admin 维护只读库（§2.7 line444 / §6.3 line1128 / §3.2 line480）

- **背景**：规格要求团队空间含「方法论库」——管理员维护、只读，含 Prompt 模板 / 画布 Schema / 方法论规则，§6.3 资产矩阵列为 Markdown/JSON 文件存放于「团队空间/方法论/」，§3.2「管理种子数据（方法论库）」仅 admin。**审计结论**：开发计划拆解V2.0.md 对该概念零任务，从未拆解为 M-任务；代码零实现（既有 "methodology" hits 全为 consultant-defense dao_layer.md 散文 + gap-check skill，无 model/route/view）；内容种子存在为文件（dao_layer.md / L1-L4.txt），但 spec line1362 明示方法论文档「独立文档保持不变」。抉择：① 文件式 vs DB；② 挂哪个模块；③ 鉴权模型；④ 种子。
- **选择 A — DB 后端 MethodologyItem 模型（非文件式）**：最大化复用 KnowledgeBase（M2.2.4 后端 + M4.2.7 前端 KnowledgeBaseView）成熟范式（category + Markdown content + CRUD + MarkdownView 渲染）。admin 维护 = 简单表单（规避文件上传 / 路径安全 / 文件树遍历复杂度）。spec 的「Markdown/JSON 文件」是 V1 存储格式描述，DB 存 Markdown 文本渲染完全等价，且 KnowledgeBase 已先例（资产矩阵同样标「Markdown 文件」但 M2.2.4 落 DB）。
- **选择 B — 挂 team_spaces 四层（model/schema/service/route 全在 reload 舞蹈内），零新 reload 行**：MethodologyItem 挂 models/team_space.py（Layer2 line57 已 reload），schemas 挂 schemas/team_spaces.py（Layer2 已 reload），service 挂 modules/team_spaces/service.py（Layer4 已 reload），路由挂 api/routes/team_spaces.py（Layer5 已 reload）——零新 reload 行，M5.5.1/#52 + M5.5.3/#54 + M5.5.6/#55 同款手法。新表 methodology_items 由 init_db create_all 自动建。
...
## 决策 #58：P6 系统设置 — 角色管理方案（角色+菜单权限，方案A）

- **背景**：用户要求新增角色管理。当前系统硬编码 3 级角色（user/admin/super），权限判断散落在 deps.py 和前端 getNavItems()。需决定角色管理粒度。
- **选择**：方案 A — 角色 + 菜单权限（非 API 级 RBAC）。Role 表定义角色，RoleMenu 多对多关联菜单可见性。后端 API 权限守卫（require_admin/require_super）继续硬编码不变，菜单可见性仅 UI 层控制。新增自定义角色只控制「能看到哪些菜单」。
- **理由**：当前系统 1-3 人使用，API 端点有限，全量 RBAC 收益不大但维护成本高。菜单级权限足够覆盖需求。

## 决策 #59：P6 系统设置 — 菜单管理数据模型（自引用树+种子数据）

- **背景**：菜单管理需支持动态配置侧边栏，替代硬编码 getNavItems()。
- **选择**：方案 A — 单表自引用树（menus 表 parent_id→menus.id），与 Organization 表同模式。内置菜单标记 is_system=True 不可删除。种子数据从现有硬编码迁移（12 条菜单项 + 4 个分组节点）。role_menus 关联表控制角色可见菜单。
- **理由**：与组织架构表同模式代码可复用；当前侧边栏本身就是三层结构，自引用树天然适配；种子数据直接迁移现有硬编码。

## 决策 #60：P6 系统设置 — 审计日志方案（关键操作审计，方案A）

- **背景**：需记录操作行为用于审计回溯。
- **选择**：方案 A — 关键操作审计日志。audit_logs 表记录写操作（create/update/delete/login/approve/reject/adopt/rollback），detail 用 JSONB 存变更前后快照。log_audit() 工具函数在各 service 写方法末尾主动调用。前端日志页面：筛选+表格+展开详情。
- **理由**：只记写操作数据量可控；JSONB detail 存变更快照满足审计需求；service 层主动调用精准可控不漏不噪。

## 决策 #61：P6 系统设置 — 字典管理暂不做

- **背景**：原计划含字典管理（分类+键值对），用户担心影响业务逻辑。
- **选择**：字典管理暂不实施。系统设置从 6 tab 缩减为 4 tab（用户管理/角色管理/菜单管理/日志管理）。
- **理由**：用户明确表示担心字典管理影响业务逻辑，优先保证核心功能稳定。后续有需求时再加不影响其他模块。

## 决策 #62：P6 系统设置 — 用户管理 tab 布局（左树右表，方案A）

- **背景**：需合并原组织架构+用户管理两个独立页面为一个 tab。
- **选择**：方案 A — 左树右表。左侧组织架构树（点击节点筛选右侧用户），右侧用户列表表格（搜索+筛选+新增+角色分配+状态管理+审批）。待审批用户在列表中高亮显示。组织架构的完整管理（增删改节点、批量导入、成员管理）集成在左侧树交互中。
- **理由**：左树右表是后台管理系统经典布局；组织架构树从原 OrganizationPage 直接迁移不丢功能；用户和组织天然关联。

## 决策 #63：P6 系统设置 — 角色菜单关联初始化策略

- **背景**：角色管理需配置每个角色能看到哪些菜单，需确定初始关联和 UI 交互。
- **选择**：种子数据自动建立关联（super→全部菜单, admin→除白名单外全部, user→作战台4+文件2+智能体管理）。UI：角色列表左→选中角色→右侧菜单树带 checkbox 勾选。系统内置角色不可删除但可修改菜单关联。super 角色菜单关联不可修改（防误操作锁死超管）。
- **理由**：种子数据与现有硬编码行为一致，迁移无感；checkbox 树交互直觉；super 不可修改是安全底线。

## 决策 #64：P6 系统设置 — 日志触发方式（Service 层主动调用，方案A）

- **背景**：审计日志 log_audit() 在哪里调用。
- **选择**：方案 A — Service 层主动调用。在各 service 写方法末尾调用 log_audit()，精准控制只记该记的，detail 可精确构造变更前后快照。
- **理由**：当前系统写操作点不多，手动覆盖完全可控；能拿到精确变更快照审计价值最高；实现最简单。

## 决策 #65：P6 系统设置 — 字典管理范围（仅展示类枚举→暂不做）

- **背景**：字典管理建好后哪些硬编码枚举要迁移。
- **选择**：方案 A — 仅迁移展示类枚举（前端下拉框选项），业务逻辑枚举不动。但因决策#61字典管理暂不做，此决策暂不执行。
- **理由**：业务状态机在代码中有硬逻辑判断必须硬编码；展示类枚举放字典后用户可扩展。但用户决定暂不做字典管理。

## 决策 #66：P6 系统设置 — 4 tab 结构

- **背景**：字典管理暂不做后，需确定最终 tab 结构。
- **选择**：4 tab — 用户管理（左树右表含组织架构+用户列表+角色分配+审批）/ 角色管理（角色列表+菜单权限勾选）/ 菜单管理（菜单树配置）/ 日志管理（审计日志查询）。组织架构完整管理集成在用户管理 tab 左侧树中，不独立 tab。
- **理由**：用户管理左侧树就是组织架构树，独立 tab 会重复；4 tab 精简不丢功能。

## 决策 #67：P6 系统设置 — 菜单加载时机（登录后一次性加载，方案A）

- **背景**：菜单管理建好后侧边栏需动态渲染，需确定菜单数据加载时机。
- **选择**：方案 A — 登录后一次性加载。用户登录成功后调 GET /api/menus 获取当前角色可见菜单树，存入前端状态。切换角色/修改菜单权限后需重新登录或手动刷新才生效。
- **理由**：菜单权限变更频率极低不需要实时同步；一次加载后侧边栏渲染无延迟；与现有 auth.me 加载模式一致。

## 决策 #68：P6 系统设置 — User.role 迁移策略（保留字符串字段，方案A）

- **背景**：新建角色表后，用户和角色的关系怎么处理。
- **选择**：方案 A — 保留 User.role 字符串字段，角色表只管菜单权限。User.role 继续存 "user"/"admin"/"super"，Role.code 与之逻辑关联（不加 FK）。新建自定义角色时 User.role 存自定义角色 code 字符串。后端 require_admin/require_super 继续检查 User.role 字符串不变。
- **理由**：User.role 字符串字段已在全系统广泛使用，改成 FK 牵一发动全身；字符串关联足够（Role.code == user.role）；零迁移成本现有用户数据不需要变更。

## 决策 #69：P6 系统设置 — 日志管理前端交互（筛选+表格+展开详情，方案A）

- **背景**：日志 tab 的查询界面设计。
- **选择**：方案 A — 筛选条件 + 时间线表格。顶部筛选栏（操作人/操作类型/目标类型/时间范围），下方表格（时间/操作人/操作/目标/摘要），点击行展开 detail（JSONB 变更快照 before→after 格式化显示）。默认最近 7 天按时间倒序。不做导出/图表。
- **理由**：筛选+表格+展开详情覆盖审计回溯核心需求；实现简单；未来需要导出/图表时再加不影响数据模型。

## 决策 #70：P6 系统设置 — 开发拆分策略（逐模块推进，方案A）

- **背景**：所有设计决策已对齐，需确定开发任务拆分。
- **选择**：方案 A — 按后端模型→后端API→前端页面逐模块推进。M6.1 角色管理后端→M6.2 菜单管理后端→M6.3 审计日志后端→M6.4 用户管理后端增强→M6.5 侧边栏动态渲染→M6.6 系统设置页面→M6.7 清理旧页面。M6.1 和 M6.3 可并行。
- **理由**：逐模块推进每步可测试可提交；与 V2.0 开发节奏一致（后端先行+前端跟进）；M6.1/M6.3 并行提效。
