...
/** 立场枚举（§7.6 态度信号 from/to） */
const STANCES = ["支持", "中立", "反对", "观望"] as const;
...
export default function VisitRecordsPage({ project, onJumpToHypothesis, onJumpToCard }: Props) {
  ...
  >(null);
  // 证据编辑弹窗（绑定到某次拜访；null=关闭）
  const [evidenceModal, setEvidenceModal] = useState<
    | { mode: "create"; visit: VisitRecord }
    | { mode: "edit"; visit: VisitRecord; evidence: EvidenceSource }
    | null
  >(null);
  ...
  const handleDeleteEvidence = useCallback(
    async (evidence: EvidenceSource) => {
      if (projectId == null) return;
      if (!window.confirm("确认删除该证据？")) return;
      try {
        await api.deleteEvidence(projectId, evidence.id);
        toast.showToast("已删除证据", "success");
        refresh();
      } catch (e) {
        toast.showToast(e instanceof Error ? e.message : "删除失败", "error");
      }
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      {/* 证据编辑弹窗（新增/编辑，绑定到指定拜访） */}
      {evidenceModal && projectId != null && (
        <EvidenceFormModal
          projectId={projectId}
          cards={cards}
          visit={evidenceModal.visit}
          evidence={"evidence" in evidenceModal ? evidenceModal.evidence : null}
...
function EvidenceFormModal({
  ...
  const toast = useToast();
  const [evidenceType, setEvidenceType] = useState<string>(evidence?.evidence_type ?? "客户原话");
  const [strength, setStrength] = useState<string>(evidence?.strength ?? "中");
  const [content, setContent] = useState(evidence?.content ?? "");
  const [sourceRoleId, setSourceRoleId] = useState<string>(evidence?.source_role_id != null ? String(evidence.source_role_id) : "");
  // §7.6 角色态度信号：携带的立场变化（from → to）
  const [fromStance, setFromStance] = useState<string>(evidence?.implied_from_stance ?? "");
  const [toStance, setToStance] = useState<string>(evidence?.implied_to_stance ?? "");
  const [saving, setSaving] = useState(false);

  const isAttitudeSignal = evidenceType === "角色态度信号";

  const save = async () => {
    if (!content.trim()) {
      toast.showToast("请填写证据内容", "error");
      return;
    }
    setSaving(true);
    const roleId = sourceRoleId ? Number(sourceRoleId) : null;
    const roleName = roleId != null ? cards.find((c) => c.id === roleId)?.name ?? null : null;
    const payload: EvidenceSourceInput = {
      ...
      // 仅角色态度信号类证据携带立场变化（§7.6）
      implied_from_stance: isAttitudeSignal && fromStance ? fromStance : null,
      implied_to_stance: isAttitudeSignal && toStance ? toStance : null,
      review_status: "reviewed",
    };
    try {
      if (evidence) {
        await api.updateEvidence(projectId, evidence.id, payload);
        toast.showToast("已更新证据", "success");
      } else {
        await api.createEvidence(projectId, payload);
        toast.showToast("已新增证据", "success");
      }
      onSaved();
    } catch (e2) {
      toast.showToast(e2 instanceof Error ? e2.message : "保存失败", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div onClick={onClose} style={modalOverlayStyle}>
      <div onClick={(e) => e.stopPropagation()} style={{ ...modalCardStyle, width: 480, maxHeight: "85vh", overflow: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
          <I.Paperclip size={16} style={{ color: "var(--accent)" }} />
          <span style={{ fontSize: 15, fontWeight: 600, fontFamily: "var(--serif)" }}>
            {evidence ? "编辑证据" : "新增证据"}
          </span>
          <span style={{ fontSize: 11, color: "var(--ink-3)" }}>· 拜访 {visit.visit_date ?? "未定日期"}</span>
          <div style={{ flex: 1 }} />
          <button onClick={onClose} style={iconBtnStyle} title="关闭">✕</button>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "flex", gap: 12 }}>
            <label style={{ ...fieldLabelStyle, flex: 1 }}>
              <span style={miniLabelStyle}>证据类型</span>
              <select value={evidenceType} onChange={(e) => setEvidenceType(e.target.value)} style={selectStyle}>
                {EVIDENCE_TYPES.map((t) => <option key={t} value={t}>{evidenceTypeLabel[t] ?? t}</option>)}
              </select>
            </label>
            <label style={{ ...fieldLabelStyle, flex: 1 }}>
              <span style={miniLabelStyle}>证据强度</span>
          ...
          <label style={fieldLabelStyle}>
            <span style={miniLabelStyle}>证据内容（引号体，必填）</span>
            <textarea value={content} onChange={(e) => setContent(e.target.value)} rows={4} style={textareaStyle} />
          </label>

          <label style={fieldLabelStyle}>
            <span style={miniLabelStyle}>来源角色（可选）</span>
            <select value={sourceRoleId} onChange={(e) => setSourceRoleId(e.target.value)} style={selectStyle}>
          ...
          {/* §7.6 角色态度信号：立场变化 */}
          {isAttitudeSignal && (
            <div style={{ padding: 12, background: "var(--bg-2)", borderRadius: 8, display: "flex", gap: 12, alignItems: "flex-end" }}>
              <label style={{ ...fieldLabelStyle, flex: 1 }}>
                <span style={miniLabelStyle}>态度变化：从</span>
                <select value={fromStance} onChange={(e) => setFromStance(e.target.value)} style={selectStyle}>
                  <option value="">（未知/无）</option>
                  {STANCES.map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
              </label>
              <span style={{ color: "var(--ink-4)", paddingBottom: 6 }}>→</span>
              <label style={{ ...fieldLabelStyle, flex: 1 }}>
                <span style={miniLabelStyle}>到</span>
                <select value={toStance} onChange={(e) => setToStance(e.target.value)} style={selectStyle}>
                  <option value="">（未知/无）</option>
                  {STANCES.map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
              </label>
        ...
        <div style={{ borderTop: "1px solid var(--line)", padding: "16px 20px", background: "var(--bg)" }}>
          ...
          <div style={{ borderTop: "1px solid var(--line)", paddingTop: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1 }} />
              <button onClick={onAddEvidence} style={ghostBtnStyle} title="新增证据">
                <I.Plus size={12} style={{ verticalAlign: -1, marginRight: 4 }} />新增证据
              </button>
            </div>
...
function EvidenceDetail({
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {(onEdit || onDelete) && (
          <div style={{ float: "right", display: "flex", gap: 2, marginLeft: 8 }}>
            {onEdit && (
              <button onClick={onEdit} style={iconBtnStyle} title="编辑证据"><I.Edit size={12} /></button>
            )}
            {onDelete && (
              <button onClick={onDelete} style={{ ...iconBtnStyle, color: "var(--danger)" }} title="删除证据"><I.Trash size={12} /></button>
            )}
          </div>
        )}
