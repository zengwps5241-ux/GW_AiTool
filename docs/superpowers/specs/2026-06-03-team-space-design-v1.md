...
当前平台已经具备个人空间文件管理、文件上传队列、Office/PDF 预览与 Markdown 转换、文件预览编辑、对话工作台和 Claude Agent 文件读写能力。PRD 中的团队空间能力需要在现有个人空间基础上补齐多人文件协作，并让绑定团队空间的会话成为团队成员可共享的工作记录。
...
6. 优化对话工作台，左侧会话列表保持按智能体分组；顶部工作空间切换决定展示个人会话或某个团队空间共享会话；新建会话时再选择智能体。
...
- 会话列表按当前工作空间过滤：个人空间只返回当前用户个人会话；团队空间返回该空间全部团队会话。
...
`ViewName` 新增 `teamSpaces`。侧边栏主导航增加：
...
用户可以创建团队空间。owner 可以编辑空间名称/描述和删除空间。
...
```text
顶部：空间名称、lock 状态、当前权限、加锁/解锁按钮、成员管理入口
...
对话工作台左侧会话栏顶部增加工作空间切换器。智能体不做常驻下拉筛选，继续作为会话列表的分组维度。新建会话时才选择智能体，避免用户浏览会话前必须先选智能体。

```text
工作空间：[个人空间 / 团队空间名称 v]    [+ 新建]
搜索：[搜索当前工作空间会话]
...
- 工作空间下拉默认选中个人空间。
- 工作空间下拉包含个人空间和当前用户加入的团队空间；团队空间项展示名称、reader/editor 权限、lock 状态和只读/可编辑状态。
- 切换工作空间后，左侧会话列表重新加载对应工作空间会话，并清空当前选中会话，避免误把个人会话内容留在团队空间视图中。
- 左侧会话列表始终按智能体分组；分组标题使用 `agent_name`，无智能体时归入“未分配”。
- 点击 `+ 新建` 后打开轻量弹出菜单或空状态选择区，让用户选择智能体；创建会话时把当前工作空间和所选智能体写入 `chat_sessions.workspace_kind/team_space_id/agent_id`。
- 新建会话的智能体默认选中平台默认智能体或用户最近使用的智能体，但不把智能体选择做成常驻筛选器。
- 已有会话继续运行时使用会话绑定的工作空间，不允许在原会话中直接切换。
- 个人空间会话只出现在创建者的个人会话列表中。
- 团队空间会话出现在该团队空间的会话列表中，当前成员都可查看历史并继续追加消息。
...
团队空间会话列表放在对话工作台 `ChatWorkspace` 的左侧会话栏中，随顶部工作空间切换器切换，不放在团队空间文件详情页里。这样用户在对话场景中先选工作空间，再按智能体分组浏览该工作空间下可继续的会话。
...
- 工作空间为个人空间时，列表展示当前用户的个人会话。
- 工作空间为团队空间时，列表展示该团队空间的共享会话，包含其他成员创建的会话。
...
- 当前用户不是任何团队空间成员时，工作空间下拉只展示个人空间，并在团队空间区域提示先创建或加入团队空间。
...
- 输入区旁展示当前工作空间权限：`可编辑`、`只读成员`、`空间已由 {name} 锁定`。
...
10. 改造对话工作台左侧会话栏：工作空间切换器、按智能体分组、点击新建后选择智能体、会话绑定字段与上传目标。
...
9. 对话工作台顶部可切换个人空间或某个团队空间；新建会话时在当前工作空间下选择智能体。
10. 绑定团队空间的会话对该空间当前成员共享，成员可查看历史并继续追加消息。
11. 对话工作台左侧会话列表随工作空间切换，个人空间展示个人会话，团队空间展示该空间共享会话。
