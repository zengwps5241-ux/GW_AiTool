...
### M4.1.6 前置分析视图（查看 + 编辑）✅ 已完成（commit 本会话）

| 任务 | 状态 | 完成时间 | 备注 |
|------|------|---------|------|
| 前置分析展示 | ✅ | 2026-07-10 | PreAnalysisView：5 维度只读卡片（行业价值链/客户地位/行业趋势/战略定位/数字化驱动力）+ 右侧维度说明 + 更新时间/作者 |
| 前置分析编辑 | ✅ | 2026-07-10 | 「编辑/填写」切表单模式，5 个 TextArea，保存调 upsertPreAnalysis（PUT /pre-analysis）|
...
### M4.1.8 节点 CRUD（新增/编辑/删除）✅ 已完成（commit 本会话）

| 任务 | 状态 | 完成时间 | 备注 |
|------|------|---------|------|
| 新增节点 | ✅ | 2026-07-10 | 「新增节点」按钮 → NodeEditModal：层级/名称/父节点/地图类型/验证状态/关联假设/置信度/来源；手动新增 review_status=reviewed 直接进正式库（§7.3） |
| 编辑节点 | ✅ | 2026-07-10 | 详情面板「编辑」按钮 → 同弹窗预填；层级不可改（避免破坏树） |
| 删除节点 | ✅ | 2026-07-10 | 详情面板「删除」→ ConfirmDialog 确认 → deleteBusinessMapObject |
| 高级 payload | ✅ | 2026-07-10 | 可选 JSON 文本框，填写则覆盖 payload；留空则合并通用字段（置信度/来源）保留既有 |
